...
public class GameMain extends JPanel implements MouseListener{
	...
	// Birthing the currentState variable into existence :)
	...
	public GameMain() {   
		...
		// assigns mouse event listener to this          
	...
	public static void main(String[] args) {
		...
		javax.swing.SwingUtilities.invokeLater(new Runnable() {
	         public void run() {
				...
				// creates a new gameMain panel and adds it to the board
				frame.add( new GameMain());	
				
				
				//sets the default close operation so that the game closes when exited
	...
	public void paintComponent(Graphics g) {
		...
		if (currentState == GameState.Playing) {          
			...
			if (currentPlayer == Player.Cross) {   
				...
				// displays the status text for which players turn it is when it is player "x" turn.
				statusBar.setText("player 'X' turn");
				
			} else {    
				
				// displays the status text for which players turn it is when it is player "0" turn.
				statusBar.setText("player '0' turn");
				
			}       
			} else if (currentState == GameState.Draw) {          
				statusBar.setForeground(Color.RED);          
				statusBar.setText("It's a Draw!Click HERE to play again");       
	  ...
	  // Initialise the game-board contents and the current status of GameState and Player) 
        ...
        public void updateGame(Player thePlayer, int row, int col) {
            ...
            if(board.hasWon(thePlayer, row, col)) {
                ...
                // checks which player won and updates the current state accordingly
                ...
                // sets the current state to draw 
	...
	public void mouseClicked(MouseEvent e) {  
		...
		// redraws the graphics of the UI        	
		repaint();
	}
